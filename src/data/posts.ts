...
const posts: Post[] = [
  {
    ...
    category: 'general',
    date: '2026.08.19',
    title: '【筆記】專案簡報架構與設計邏輯',
    excerpt: '如何向評審與教授清晰表達專案價值？整理專案發表的說服邏輯與視覺排版心得。',
    content: `
      <p class="post-intro">
        專題發表或競賽簡報時，最重要的就是邏輯順暢與重點清晰。這篇整理我在準備專案發表時歸納的三個核心段落與排版技巧。
      </p>

      <h2>釐清核心痛點與動機</h2>
      <p>
        簡報開頭必須先建立情境與說服力。以校園二手書或物品媒合為例，痛點往往是社團發文容易被洗掉、缺乏身分驗證導致信任不足。點出問題的真實性與場域集中性，聽眾才能快速理解專案存在的必要。
      </p>

      <h2>具體解法與產品核心價值</h2>
      <p>
        不單純做資料儲存，而是著重於信任機制的建立與體驗優化。例如導入 PWA 讓行動裝置能隨開即用、搭配通知機制與信箱聯繫，確保重要交易訊息不漏接，讓媒合流程具備完整追蹤性。
      </p>

      <h2>技術選型與工程實現</h2>
      <p>
        說明為何選擇特定的前後端技術組合。現代 Web 應用需要良好的狀態管理與即時反饋，選用合適的工具搭配具體業務場景，系統運作才能兼顧效能與開發效率。
      </p>

      <h2>視覺排版與呈現細節</h2>
      <ul>
        <li><strong>導覽標籤提示：</strong>在角落放置流程指示，讓聽眾隨時掌握當前進度。</li>
        <li><strong>結論先行：</strong>頁面標題直接點出該頁核心結論，減少閱讀理解成本。</li>
        <li><strong>清晰的資訊層次：</strong>透過圖示搭配簡要重點列，保持畫面乾淨易讀。</li>
      </ul>

      <p class="post-outro">
        掌握痛點、解法與實現這三個核心環節，簡報就能有條理地傳達專案真正的價值。
      </p>
    `,
  },
  {
